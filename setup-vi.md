...
Áp dụng cho VisionDesk **1.0.4** (`v1.0.4`).
...
Production cài từ tag cố định `v1.0.4`, không dùng `master`. `master` chỉ dành
...
Nếu installer báo graphical session riêng của `visiondesk` chưa hoạt động, hãy
reboot một lần. Installer chủ động giữ service ở trạng thái dừng thay vì cố mở
X11 session của user quản trị khi chưa được cấp quyền.
...
Chạy `update.sh` từ source checkout trùng với release đích. Riêng bản sửa
service unit `1.0.3 -> 1.0.4`, cần fetch và checkout `v1.0.4` trước khi gọi
updater.
