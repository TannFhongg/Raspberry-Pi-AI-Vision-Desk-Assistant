...
This document defines the appliance update package for VisionDesk 1.0.0. It is
...
agree after normalizing `v1.0.0` to application version `1.0.0`. It writes:

```text
dist/visiondesk-1.0.0.tar.gz
```
...
```bash
scripts/verify-release.sh dist/visiondesk-1.0.0.tar.gz --expected-version 1.0.0
sha256sum dist/visiondesk-1.0.0.tar.gz
```
...
Attach the verified `dist/visiondesk-1.0.0.tar.gz` to the existing GitHub Release
tagged `v1.0.0`. With GitHub CLI installed and authenticated:

```bash
gh release upload v1.0.0 dist/visiondesk-1.0.0.tar.gz
```
