"""Shared text-only result history storage for Flask and Qt frontends."""
...
class ResultHistoryStore:
    """Load, validate, retain, and append recent text-only result history entries."""
    ...
    def load_entries(self) -> list[dict[str, Any]]:
        ...
        with self._lock:
            if self._cache is not None:
                return [entry.copy() for entry in self._cache]
    ...
    def write_entries(self, entries: list[dict[str, Any]]) -> None:
        ...
        normalized_entries = self.apply_retention(
            [
                entry.copy()
                for entry in entries[: self.result_limit]
                if self.coerce_entry(entry) is not None
            ]
        )
